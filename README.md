...
Команда — **🅰️ выше**. Установщик спросит только **домен кабинета** (остальное
берётся из конфигурации бота) и поднимет весь стек.

После установки добавьте домен кабинета в ваш reverse-proxy (TLS → `127.0.0.1:5002`):
...
Команда — **🅱️ выше** (устанавливает Docker, Caddy, код, кабинет и TLS на 443;
запросит 3 значения).

> ⚠️ **Сначала установите кабинет на сервере бота** (Сценарий А) — он добавляет
> API-эндпоинты, к которым обращается отдельный кабинет. Если бот уже работает с
> кабинетом — этот шаг не нужен.

> Требуется сервер со **свободным портом 443** и доменом кабинета, направленным
> на его IP (откройте у провайдера входящие TCP 80 и 443).

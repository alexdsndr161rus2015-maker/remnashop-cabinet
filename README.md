...
- Если **443 занят** (у вас уже свой reverse-proxy) — скрипт его не трогает и
  просто показывает готовый блок для вставки в ваш прокси на `127.0.0.1:5002`.
...
> ⚠️ Если в «Web Login» предложат одноразовый переход на **чистый OpenID Connect** —
> **не включайте** его: кабинет использует классический Telegram Login Widget, и
> этот тумблер (он необратимый) его сломает. Достаточно просто добавить адрес в
> **Allowed URLs**.
...
> доступен вход **по email**. Скрипт этот шаг не делает — он на стороне Telegram.

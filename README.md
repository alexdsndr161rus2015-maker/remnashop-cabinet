...
[RемнаShop](https://remnashop.mintlify.app/) на базе [Remnawave](https://remna.st/).**
...
Дополнение к боту RемнаShop, которое добавляет два больших модуля:
...
- **Уже настроенный бот RемнаShop** — заполненный `.env`
...
1. Направьте домены (бота и кабинета) на сервер.
2. Поднимите reverse-proxy с TLS (Caddy / nginx) на порты `5000` и `5002`.

<details>
<summary>Пример для Caddy</summary>
...
| **Добавлено RемнаShop** | блок, который дописывает `install.sh` |
...
> темы. Значение по умолчанию для названия — `RемнаShop`.
...
Сначала настройте бота RемнаShop (`cp .env.example .env`, заполните раздел

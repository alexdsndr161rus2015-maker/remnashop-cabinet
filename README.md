...
ей нужно ~1–1.5 ГБ RAM единоразово. Отсюда:

| Параметр | Минимум | Рекомендуется |
|----------|---------|---------------|
| ОС | Ubuntu 20.04 | Ubuntu 22.04+ |
| RAM | **1 ГБ** (+swap для сборки) | **2 ГБ** |
| CPU | 1 vCPU | 2 vCPU |
| Диск | 10 ГБ | 15–20 ГБ |
| Канал | по числу юзеров кабинета (это API-трафик, не VPN) | 100 Mbps |

> На **1 ГБ RAM** сборка Vite может упасть по OOM — заранее добавьте swap
> (`fallocate -l 2G /swapfile && mkswap /swapfile && swapon /swapfile`) или
> соберите образ на машине помощнее и перенесите. После сборки кабинет спокойно
> живёт на 1 ГБ. Сам **VPN-трафик через этот сервер НЕ идёт** — он только отдаёт
> кабинет и проксирует API, поэтому мощный канал/CPU не нужны.
